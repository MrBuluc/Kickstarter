...
describe("Campaigns", () => {
  ...
  it("allows a manager to make a payment request", async () => {
    await campaign.methods.createRequest("Buy PS4", "100", accounts[1]).send({
      from: accounts[0],
      gas: "1000000",
    });
    const request = await campaign.methods.requests(0).call();

    assert.equal("Buy PS4", request.description);
    assert.equal("100", request.value);
    assert.equal(accounts[1], request.recipient);
    assert(!request.complete);
    assert.equal(0, request.yesCount);
    assert.equal(0, request.noCount);
  });
  ...
  it("proceses requests", async () => {
    ...
    await campaign.methods
      .createRequest("Buy PS5", web3.utils.toWei("5", "ether"), accounts[2])
      .send({ from: accounts[0], gas: "1000000" });
